<?php
	...
	if(!empty($_POST['c_pw']) && !empty($_POST['n_pw']) && !empty($_POST['cn_pw']))
	{
		if($_POST['n_pw']==$_POST['cn_pw'])
		{
			...
			$jms_select_sql = "SELECT * FROM `user-registration` WHERE jms_password=:c_pw and id=:jms_user_id";
			...
			if(count($jms_row) == 1)
			{
		        ...
		        $jms_updated_pass = "UPDATE `user-registration` SET jms_password = :n_pw,updated_on=:updated_on WHERE id='$jms_user_id'";
