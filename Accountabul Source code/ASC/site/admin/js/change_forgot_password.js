...
{ 
    ...
    { 
        ...
        if(validatelog_inForm())
        {
            enablelog_inForm();
            $.ajax({
                url: "admin_data_file/change_forgot_password_data.php",
                type: "POST",
                ...
                success: function(data)
                {
                    ...
                    if(data.status == 'error')
                    {
                        ...
                        $('.jms-change-forgot-message').html("<div class='alert alert-danger m-0' role='alert'>"+data.message+"</div>");
                    }
                    else if(data.status == 'success')
                    {
                        $('.jms-class-none').css('display','none');
                        $('.jms-change-forgot-message').css('display', 'block');
                        $('.jms-change-forgot-message').html("<div class='alert alert-success m-0' role='alert'>"+data.message+"</div>");
